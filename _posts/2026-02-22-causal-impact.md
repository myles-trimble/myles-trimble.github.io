...
image: "/posts/causal-impact-title-img-2.png"
...
We saw a 41.1% uplift in sales for those customers that joined the Delivery Club over and above what we believe they would have spent, had the club not existed.  This was across the three month post-period and the uplift was deemed to be statistically significant (@ 95%).
...
Depending on the industry, this could be the uplift in sales after a promotion or a product release, the additional clicks, conversions, or signups generated by an online ad campaign, or the change in share price after a market event.
...
Once we have this counterfactual, we can proceed to calculate the estimation for the causal effect.
...
In the code below, we specify the start and end dates of the "pre-period" and of the "post-period". We then apply the algorithm by passing in the DataFrame and the specified pre- and post-period time windows.
...
The bottom chart shows the cumulative uplift over time - it is effectively adding up the Pointwise contributions from the second chart over time. This is very useful as it helps the viewer get a feel for what the total uplift or difference is at any point in time.
